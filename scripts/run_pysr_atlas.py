#!/usr/bin/env python3
"""
PySR with Atlas Metalífero ground truth (field-mapped alteration zones).
NO circular argument: classes come from geological field mapping, not spectral indices.
"""
...
def main():
    print("Loading Atlas ground truth training data...")
    data = np.load(GT_DIR / "maricunga_training_atlas_20m.npz", allow_pickle=True)
    X = data["X"]
    y = data["y"]
    ...
    for cls_id in np.unique(y):
        n = (y == cls_id).sum()
        if n < MIN_SAMPLES:
            name = CLASSES.get(int(cls_id), f"Class_{cls_id}")
    ...
    for cls_id, cls_name in CLASSES.items():
        ...
        if n > 0:
            print(f"  {cls_id} {cls_name:25s}: {n:6d}")
    ...
    for cls_id, cls_name in CLASSES.items():
        ...
        model = PySRRegressor(
            ...
            tempdir=str(RESULTS_DIR / "pysr_temp_atlas"),
            turbo=True,
            bumper=True,
        )
        ...
        if equations_df is not None:
            equations_df.to_csv(RESULTS_DIR / f"equations_atlas_{cls_name}.csv", index=False)
        ...
        all_results[cls_name] = {
            ...
            "ground_truth_source": "Atlas Metalífero III Región, SERNAGEOMIN (field-mapped)",
        }
    ...
    with open(RESULTS_DIR / "pysr_results_atlas.json", 'w') as f:
        json.dump(all_results, f, indent=2)

    print("\n" + "=" * 70)
    print("SUMMARY — Field-Validated Spectral Indices (Atlas Metalífero)")
    print("Ground truth: SERNAGEOMIN geological field mapping")
    print("=" * 70)
